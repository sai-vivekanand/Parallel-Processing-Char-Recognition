...
def train_model_on_partition(partition_id, X_train_part, y_train_part, X_val, y_val, config):
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        num_workers=0,  # Important: keep this at 0 for Dask
        pin_memory=True
    )
...
def run_data_parallel_training(X_train, y_train, X_val, y_val, X_test, y_test, config):
    ...
    # Set up dask client with the specified number of workers
    client = Client(n_workers=config.cpus)
    ...
    for i in range(config.cpus):
        future = dask.delayed(train_model_on_partition)(
...
def plot_and_save_performance_metrics(all_results, cpu_configs, batch_sizes, base_times):
    ...
    # Save to CSV
    csv_filename = f'results/dask_experiment_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
...
if __name__ == '__main__':
    ...
    batch_sizes = [32, 64, 128]  # Test with three batch sizes
    epochs = 5
